...
FILE_PATH="./bin/tidb-server-linux"
HUB_PATH="csn/tidb"
...
# Since we are cross-compiling on Mac for Linux, we cannot execute the binary to get the hash.
...
echo "$dockerFile"| docker build --platform linux/amd64 -t ${IMAGE} -f- .
