...
func EncodeResourceGroupTag(sqlDigest, planDigest *parser.Digest, label tipb.ResourceGroupTagLabel, tableName, schema string) []byte {
	...
	if len(tableName) > 0 {
		tag.TableName = []byte(tableName)
	}
	if len(schema) > 0 {
		tag.Schema = []byte(schema)
	}
	b, err := tag.Marshal()
	if err != nil {
		return nil
